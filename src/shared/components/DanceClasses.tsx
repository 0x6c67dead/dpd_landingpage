...
interface DanceClassesProps {
  classes?: DanceClassItem[];
}

export default function DanceClasses({ classes }: DanceClassesProps) {
  const displayClasses = classes && classes.length > 0 ? classes : [];
      ...
      <div className="container mx-auto px-5 md:px-12 lg:px-24 z-10 relative">
        ...
        <div className="mt-16 flex flex-col gap-0">
          {displayClasses.map((cls, idx) => {
            const accent = cls.accent_color || cls.accentColor || "primary";
            const message = cls.whatsapp_message || cls.whatsappMessage || "";

            return (
              <motion.div
                key={cls.id || cls.number}
                ...
                className={`
                  ...
                  ${idx === displayClasses.length - 1 ? "border-b" : ""}
                  group
                `}
              >
                {/* Number Column */}
                <div className="w-full lg:w-32 flex-shrink-0 flex items-center justify-start lg:justify-center py-4 lg:py-16 px-0 lg:px-0">
                  <span className={`text-5xl md:text-7xl lg:text-8xl font-display font-black leading-none ${
                    accent === "primary" ? "text-primary/30 group-hover:text-primary" : "text-secondary/30 group-hover:text-secondary"
                ...
                <div className="flex-1 flex flex-col lg:flex-row gap-6 lg:gap-16 py-2 lg:py-16 px-0 lg:px-8">
                  ...
                  <div className="w-full lg:w-1/3 flex flex-col gap-3">
                    ...
                    <div className={`inline-flex items-center gap-2 mt-2 px-4 py-2 border ${
                      accent === "primary" ? "border-primary/40 text-primary" : "border-secondary/40 text-secondary"
